...
rem disable_path_overrides=no: exported exes must accept --path (running a
rem project from a different directory than the pck/exe).
scons p=windows target=template_debug precision=single module_mono_enabled=yes module_text_server_fb_enabled=yes mono_glue=yes disable_path_overrides=no || goto :error

echo Building export templates (release)...
scons p=windows target=template_release precision=single module_mono_enabled=yes module_text_server_fb_enabled=yes mono_glue=yes disable_path_overrides=no || goto :error
